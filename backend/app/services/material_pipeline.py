...
async def build_material_knowledge_base(
    ...
    logger.info("[%s] 开始建库", material.filename)
    ...
    if material.file_type.lower() == "pdf":
        ...
        logger.info("[%s] 准备 OCR", material.filename)
        ...
        if ocr_result is not None:
            ...
            logger.info("[%s] 正在切分 OCR 文本", material.filename)
            ...
            logger.info("[%s] 提取 PDF 内置文本", material.filename)
        ...
        logger.info("[%s] 解析并切分文档", material.filename)
        text = extract_text(material.storage_path)
        chunks = split_text_into_chunks(
            text,
            chunk_size=settings.chunk_size,
    ...
    logger.info("[%s] 生成摘要与关键词：%d 个 chunk", material.filename, len(chunks))
    ...
    logger.info("[%s] 写入 Chroma 知识库：%d 个 chunk", material.filename, len(chunks))
    ...
    material = crud.replace_material_chunks(
